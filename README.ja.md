...
Claude Code の `/fast` は割増単価で課金されます（Opus 5 / Opus 4.8 は $5/$25 ではなく
**$10/$50**、Opus 5.5 は $4/$20 ではなく **$8/$40**）。キャッシュ倍率は fast 単価に対して
適用されます。transcript の
...
- **単価**: `[pricing.models."<id>"]`。省略したフィールドは **継承** されます
  ...
  2行だけの上書きで十分です。未知のモデルを足すときは価格ページのキャッシュ読み取り
  の脚注を確認してください。Fable 5.1 / Mythos 5.1 は 0.025×、Opus 5.5 は
  0.05× で標準の 0.1× ではありません。同じ扱いのモデルには `cache_read_multiplier` の明示が必要です。蓄積済み
  レコードに反映するには、そのあと `reprice` を実行してください。
